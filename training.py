...
class SeqEHRTrainer(object):
    ...
    def predict(self, test_data_loader, do_eval=True):
        ...
        if self.args.result_path:
            ...
            with open(raw_res_fn, "w") as f:
                header = "\t".join(["\t".join(range(len(yp_probs[0]))), "predict_label", "true_label"])
                f.write(header + "\n")
                for each in zip(yp_probs, yp_tags, yt_tags):
                    probs = "\t".join([str(e) for e in each[0]])
                    line = "\t".join([probs, each[1], each[2]]) + "\n"
                    f.write(line)
    ...
    def _save_model(self):
        new_model_bin_file = Path(self.args.new_model_path) / "pytorch_model.{}.bin".format(self.args.model_type)
        model_to_save = self.model.module if hasattr(self.model, 'module') else self.model
        torch.save(model_to_save.state_dict(), new_model_bin_file)

        new_model_config_file = Path(self.args.config_path) / "config.{}.pkl".format(self.args.model_type)
        pkl_save(self.config, new_model_config_file)

    def _load_model(self):
        config_path = Path(self.args.config_path) / "config.{}.pkl".format(self.args.model_type)
        self.config = pkl_load(config_path)

        self.model = MixModel(config=self.config, model_type=self.args.model_type)
        model_path = Path(self.args.new_model_path) / "pytorch_model.{}.bin".format(self.args.model_type)
    ...
    def _eval(self, batch_iter):
        ...
        for step, batch in enumerate(batch_iter):
            batch = tuple(b.to(self.args.device) for b in batch[:-1])
            with torch.no_grad():
                ...
                if yt_probs is None:
                    ...
                    yt_tags = np.concatenate([yt_tags, true_probs[1]], axis=0)
